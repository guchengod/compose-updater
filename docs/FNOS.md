...
- fnOS 版本不低于 `1.2.0203`；该最低版本基于 x86_64 真实设备验证，每个正式 FPK 仍需在发布前回归；
...
```bash
FNPACK=/usr/local/bin/fnpack make fnos-build VERSION=v0.6.4
```

产物：

```text
build/fnos/ComposeUpdater-v0.6.4-x86.fpk
build/fnos/ComposeUpdater-v0.6.4-arm.fpk
```
...
构建脚本会从 `CHANGELOG.md` 中读取对应版本的条目写入 manifest，也可以用 `PACKAGE_CHANGELOG` 覆盖；`OS_MIN_VERSION` 可覆盖最低 fnOS 版本，但只能填写真实测试过的兼容范围。
